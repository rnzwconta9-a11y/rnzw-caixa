...
struct PatchProjectsView: View {
    ...
    init(managementEnabled: Bool = true, displayTitle: String? = nil) {
        self.managementEnabled = managementEnabled
        self.displayTitle = displayTitle
#if targetEnvironment(simulator)
        _showCreate = State(
            initialValue: managementEnabled && ProcessInfo.processInfo.arguments.contains("--simulate-patch-editor")
        )
#endif
    }
    ...
    var body: some View {
        NavigationStack {
            ...
            .navigationTitle(displayTitle ?? language.text("patch.title"))
            ...
            .onAppear {
                if managementEnabled { consumeExternalImport() }
            }
    ...
    private func itemRow(_ item: PatchLibraryItem) -> some View {
        if item.isLocked {
            ...
            NavigationLink {
                PatchProjectDetailView(store: store, projectID: item.id, managementEnabled: managementEnabled)
            } label: {
                PatchProjectRow(item: item, language: language)
            }
        }
    ...
    var body: some View {
            ...
            if let item, let project = item.project {
                if isWorkspaceProject {
                    Section {
                        ...
                        if managementEnabled, let workspaceURL = item.workspaceURL {
                            NavigationLink {
                                FileBrowserView(
        ...
        .toolbar {
            ToolbarItem(placement: .navigationBarTrailing) {
                if isWorking {
                    ProgressView()
                } else if managementEnabled && !isWorkspaceProject {
        ...
        .sheet(isPresented: $showEditor) {
            if managementEnabled, let item, let project = item.project {
                PatchProjectEditorView(
